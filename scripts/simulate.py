...
def simulate_dataset(
    ...
    for i, axi in enumerate(ax.flat):
        ...
        if i == n_subplots - 1:
            break
    plt.suptitle("Sample Concentration Curves", fontsize=15)
    plt.tight_layout()
    ...
    fig_b, axs = plt.subplots(1,3, dpi=300)
    width=150 #mm
    ...
    for i, axi in enumerate(axs.ravel()):
        ...
        axi.hist(params[:,j], bins=100, alpha=1.0)
        axi.set_title(param_names[j], fontsize=11)
        # set tick fontsize
        axi.tick_params(axis='both', which='major', labelsize=6)
        # set label fontsize
        axi.set_xlabel("Value", fontsize=6)
        axi.set_ylabel("Frequency", fontsize=6)
    plt.suptitle("Parameter Histograms", fontsize=15)
